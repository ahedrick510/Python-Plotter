            ...
            for file, df in self.dataframes.items():
                for (y_col, label) in self.columns[file]:
                    ...
                    if n != len(df[y_col]):
                        ...
                        output_resampled *= -1  # Invert output signal for DLT
                        print("Inverting DLT output signal for Bode plot")
                    ...
                    if color_counter % 2 == 0:
                        PVDF_MULTIPLIER = 17     # found by trial and error based on PX_to_DEG

                        # notch filter at 60 Hz to remove noise from power lines for PVDF data
                        b, a = iirnotch(60, 150, self.sampling_rate)
                        output_signal = filtfilt(b, a, output_signal)
                        output_fft = np.fft.rfft(output_signal)

                        output_fft = output_fft * PVDF_MULTIPLIER  # Adjust amplitude for visibility
                        print("Scaling up PVDF FFT to match DLT results")

                    else:
                        output_fft = np.fft.rfft(output_signal)
                        PX_to_DEG = 4.04   # conversion factor from pixels to degrees for FLAAIR v11p4 and PVDF Wings Testing\2026-03-25 FLAAIR 14 NOT v11p4, determined experimentally
                        output_fft = output_fft * PX_to_DEG  # Adjust amplitude for visibility
                        print("Scaling DLT FFT amplitude to convert to deg")
